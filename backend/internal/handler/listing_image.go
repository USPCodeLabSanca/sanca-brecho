...
func generatePresignedURL(c *gin.Context) {
	...
	// Decide on the S3 key (you might want to prefix with a folder or timestamp)
	// A common pattern is: `uploads/{timestamp}_{randomUUID}_{originalFilename}`.
	//  Here, for simplicity, let’s just use exactly what client said:
	key := req.Filename

	// 2.2. Build the PutObjectInput
	...
	// 2.3. Ask the presignClient to generate a presigned URL valid for, say, 15 minutes
	presignedReq, err := presignClient.PresignPutObject(context.TODO(), input,
		manager.WithPresignExpires(15*time.Minute),
	)
	if err != nil {
	...
	// 2.4. Build the “public URL” (this assumes you’re using the standard S3 endpoint
	//      and the object is ACL=public-read). If you’re using a CloudFront distribution
	//      or a custom domain, swap this out accordingly.
	publicURL := fmt.Sprintf("https://%s.s3.%s.amazonaws.com/%s", config.BucketName, config.AwsRegion, key)
