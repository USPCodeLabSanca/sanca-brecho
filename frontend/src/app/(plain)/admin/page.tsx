...
import { getAllUsers, deleteUser } from "@/lib/services/userService";
                ...
                <tbody>{listings.map((listing) => (
                    <tr key={listing.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="p-3 text-center">
                            <button onClick={() => setListingModalState({ listing })} className="text-red-500 hover:text-red-700 p-1" title="Deletar Anúncio">
                ...
                <tbody>{users.map((user) => (
                    <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="p-3 flex gap-2 justify-center">
                            <button onClick={() => handleDeleteUser(user.slug)} className="text-red-500 hover:text-red-700 p-1" title="Deletar Usuário"><Trash2 className="w-5 h-5" /></button>
                            <button onClick={() => setUserModalState({ user })} className="text-blue-500 hover:text-blue-700 p-1" title="Alterar Cargo"><UserCog className="w-5 h-5" /></button>
                        </td>
                    </tr>
                ))}</tbody>
            </table></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <section className="bg-white p-6 rounded-lg shadow-md">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setCategoryModalState({ type: "add" })} className="bg-sanca text-white px-3 py-1.5 rounded-md flex items-center gap-2 hover:opacity-90 text-sm font-medium">
                ...
                <ul className="space-y-2">{categories.map((cat) => (
                    <li key={cat.id} className="flex justify-between items-center p-3 border border-gray-100 rounded-md hover:bg-gray-50">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setCategoryModalState({ type: "edit", category: cat })} className="text-blue-500 hover:text-blue-700"><Edit className="w-4 h-4"/></button>
                            <button onClick={() => setCategoryModalState({ type: "delete", category: cat })} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4"/></button>
                        </div>
                    </li>
                ))}</ul>
            </section>
            ...
            <section className="bg-white p-6 rounded-lg shadow-md">
                ...
                <ul className="space-y-3">{reports.length > 0 ? (reports.map((report) => (
                    <li key={report.id} className="p-3 border border-gray-100 rounded-md">
                        ...
                        <div className="mt-2">
                            <button onClick={() => setReportModalState({ report })} className="text-sm bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600">
                                Analisar Denúncia
                            </button>
                        </div>
                    </li>
